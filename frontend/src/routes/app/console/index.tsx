...
export const ConsolePage: React.FC<ConsolePageProps> = ({ onNavigateToStudentDetail }) => {
  ...
  const handleSimulateDrift = async () => {
    setIsDrifting(true);
    try {
      await apiClient.post('/demo/drift-hero');
      toast('Drift Simulated', 'Aisha has natural drift. Recalculating risk...', 'guard');
      await queryClient.invalidateQueries({ queryKey: ['cohort'] });
      await queryClient.invalidateQueries({ queryKey: ['state'] });
      await queryClient.invalidateQueries({ queryKey: ['plan'] });
    } catch (err: any) {
      toast('Drift Simulation Failed', err.message || 'Error triggering drift', 'error');
    } finally {
      setIsDrifting(false);
    }
  };
  ...
  const handleResetDemo = async () => {
    ...
    try {
      ...
      toast('Database Reset', 'Cohort data restored to default baseline.', 'guard');
      await queryClient.invalidateQueries({ queryKey: ['cohort'] });
      await queryClient.invalidateQueries({ queryKey: ['state'] });
      await queryClient.invalidateQueries({ queryKey: ['plan'] });
    } catch (err: any) {
      toast('Reset Failed', err.message || 'Error resetting demo data', 'error');
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="shrink-0 flex items-center space-x-2">
          <button
            ...
            disabled={isDrifting || isResetting}
            className="px-4 py-2 bg-risk-high hover:bg-risk-high/90 text-white rounded font-semibold text-xs flex items-center gap-1.5 transition cursor-pointer shadow-lg disabled:opacity-50"
          >
            <span>{isDrifting ? 'Drifting...' : 'Simulate Drift'}</span>
          </button>
          <button
            onClick={handleResetDemo}
            disabled={isDrifting || isResetting}
            className="px-4 py-2 bg-text hover:bg-text/90 text-bg rounded font-semibold text-xs flex items-center gap-1.5 transition cursor-pointer shadow-lg disabled:opacity-50"
          >
